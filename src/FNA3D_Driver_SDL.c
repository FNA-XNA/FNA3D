...
static SDL_GpuVertexElementFormat XNAToSDL_VertexAttribType[] =
{
	...
	SDL_GPU_VERTEXELEMENTFORMAT_BYTE4,		/* FNA3D_VERTEXELEMENTFORMAT_BYTE4 */ /* FIXME: NOT SCALED */
...
static FNA3D_Device* SDLGPU_CreateDevice(
	...
	SDLGPU_Renderer *renderer;
	SDL_GpuDevice *device;
	SDL_PropertiesID deviceProperties;
	SDL_GpuSwapchainComposition swapchainComposition;
	SDL_GpuTextureCreateInfo textureCreateInfo;
	SDL_GpuSamplerCreateInfo samplerCreateInfo;
	SDL_GpuTransferBufferCreateInfo transferBufferCreateInfo;
	...
	deviceProperties = SDL_CreateProperties();
	SDL_SetBooleanProperty(deviceProperties, SDL_PROP_GPU_CREATEDEVICE_DEBUGMODE_BOOL, debugMode);
	SDL_SetBooleanProperty(deviceProperties, SDL_PROP_GPU_CREATEDEVICE_VULKAN_SCALEVERTEXELEMENTS, SDL_TRUE);
	/* FIXME: We can only support DXIL after fixing https://github.com/icculus/mojoshader/issues/68 */
	SDL_SetBooleanProperty(deviceProperties, SDL_PROP_GPU_CREATEDEVICE_SHADERS_SPIRV_BOOL, SDL_TRUE);
	SDL_SetBooleanProperty(deviceProperties, SDL_PROP_GPU_CREATEDEVICE_SHADERS_MSL_BOOL, SDL_TRUE);
	device = SDL_GpuCreateDeviceWithProperties(deviceProperties);
