...
static SDL_GpuTextureFormat XNAToSDL_SurfaceFormat[] =
{
	...
	SDL_GPU_TEXTUREFORMAT_R8G8B8A8_SRGB,		/* SurfaceFormat.ColorSrgbEXT */
	SDL_GPU_TEXTUREFORMAT_BC3_SRGB,			/* SurfaceFormat.Dxt5SrgbEXT */
	SDL_GPU_TEXTUREFORMAT_BC7,			/* SurfaceFormat.Bc7EXT */
	SDL_GPU_TEXTUREFORMAT_BC7_SRGB			/* SurfaceFormat.Bc7SrgbEXT */
};
...
static void SDLGPU_SwapBuffers(
	...
	{
		srcRegion.textureSlice.texture = renderer->fauxBackbufferColor->texture;
...
static void SDLGPU_INTERNAL_BeginRenderPass(
	...
	{
		colorAttachmentInfos[i].textureSlice.texture = renderer->nextRenderPassColorAttachments[i]->texture;
	...
	{
		depthStencilAttachmentInfo.textureSlice.texture = renderer->nextRenderPassDepthStencilAttachment->texture;
...
static void SDLGPU_ResolveTarget(
	...
	SDL_GpuGenerateMipmaps(renderer->copyPass, texture->texture);
}
...
static SDL_GpuGraphicsPipeline* SDLGPU_INTERNAL_FetchGraphicsPipeline(
	...
	hash.colorFormats[0] = SDL_GPU_TEXTUREFORMAT_R8G8B8A8;
	hash.colorFormats[1] = SDL_GPU_TEXTUREFORMAT_R8G8B8A8;
	hash.colorFormats[2] = SDL_GPU_TEXTUREFORMAT_R8G8B8A8;
	hash.colorFormats[3] = SDL_GPU_TEXTUREFORMAT_R8G8B8A8;
	...
	createInfo.multisampleState.multisampleCount = renderer->nextRenderPassMultisampleCount;
...
static void SDLGPU_INTERNAL_SetTextureData(
	...
	SDL_GpuTransferBufferRegion transferRegion;
	...
	transferRegion.transferBuffer = transferBuffer;
	transferRegion.offset = transferOffset;
	transferRegion.size = dataLength;
...
static void SDLGPU_INTERNAL_GetTextureData(
	...
	transferRegion.transferBuffer = renderer->textureDownloadBuffer;
	transferRegion.offset = 0;
	transferRegion.size = dataLength;
...
static void SDLGPU_INTERNAL_GetBufferData(
	...
	/* Copy into data pointer */
	transferRegion.transferBuffer = renderer->bufferDownloadBuffer;
	transferRegion.offset = 0;
	transferRegion.size = dataLength;
...
static FNA3D_Device* SDLGPU_CreateDevice(
	...
	renderer->supportsDXT1 = SDL_GpuIsTextureFormatSupported(
		renderer->device,
		SDL_GPU_TEXTUREFORMAT_BC1,
		SDL_GPU_TEXTURETYPE_2D,
		SDL_GPU_TEXTUREUSAGE_SAMPLER_BIT
	);
	renderer->supportsBC2 = SDL_GpuIsTextureFormatSupported(
		renderer->device,
		SDL_GPU_TEXTUREFORMAT_BC2,
		SDL_GPU_TEXTURETYPE_2D,
		SDL_GPU_TEXTUREUSAGE_SAMPLER_BIT
	);
	renderer->supportsBC3 = SDL_GpuIsTextureFormatSupported(
		renderer->device,
		SDL_GPU_TEXTUREFORMAT_BC3,
		SDL_GPU_TEXTURETYPE_2D,
		SDL_GPU_TEXTUREUSAGE_SAMPLER_BIT
	);
	renderer->supportsBC7 = SDL_GpuIsTextureFormatSupported(
		renderer->device,
		SDL_GPU_TEXTUREFORMAT_BC7,
		SDL_GPU_TEXTURETYPE_2D,
		SDL_GPU_TEXTUREUSAGE_SAMPLER_BIT
	);
	renderer->supportsSRGB = SDL_GpuIsTextureFormatSupported(
		renderer->device,
		SDL_GPU_TEXTUREFORMAT_R8G8B8A8_SRGB,
		SDL_GPU_TEXTURETYPE_2D,
		SDL_GPU_TEXTUREUSAGE_COLOR_TARGET_BIT | SDL_GPU_TEXTUREUSAGE_SAMPLER_BIT
	);
	renderer->supportsD24 = SDL_GpuIsTextureFormatSupported(
		renderer->device,
		SDL_GPU_TEXTUREFORMAT_D24_UNORM,
		SDL_GPU_TEXTURETYPE_2D,
		SDL_GPU_TEXTUREUSAGE_DEPTH_STENCIL_TARGET_BIT
	);
	renderer->supportsD24S8 = SDL_GpuIsTextureFormatSupported(
		renderer->device,
		SDL_GPU_TEXTUREFORMAT_D24_UNORM_S8_UINT,
		SDL_GPU_TEXTURETYPE_2D,
		SDL_GPU_TEXTUREUSAGE_DEPTH_STENCIL_TARGET_BIT
